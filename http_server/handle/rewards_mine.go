...
func (h *HttpHandle) RewardsMine(ctx *gin.Context) {
	...
	if err = h.doRewardsMine(&req, &apiResp); err != nil {
		log.Error("doRewardsMine err:", err.Error(), funcName, clientIp, ctx)
	}
...
func (h *HttpHandle) doRewardsMine(req *ReqRewardsMine, apiResp *api_code.ApiResp) error {
	...
	list, err := h.dbDao.GetMyRewards(addressHex.ChainType, addressHex.DasSubAlgorithmId, addressHex.AddressHex, tables.ServiceTypeRegister, tables.RewardTypeInviter, req.GetLimit(), req.GetOffset())
	if err != nil {
	...
	rc, err := h.dbDao.GetMyRewardsCount(addressHex.ChainType, addressHex.DasSubAlgorithmId, addressHex.AddressHex, tables.ServiceTypeRegister, tables.RewardTypeInviter)
	if err != nil {
