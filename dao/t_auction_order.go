...
WHERE o.account= "%s" and p.status != %d and o.bid_time > %d `, tables.TableNameAuctionOrder, tables.TableNameRegisterPendingInfo, account, tables.StatusRejected, createTime)
	err = d.db.Raw(sql).Find(&list).Error
	return list, nil
}
